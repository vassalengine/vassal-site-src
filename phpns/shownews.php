...
if (isset($phpns['freeze_file']) && strstr($phpns['freeze_file'], 'freeze.')) {
  ...
  // set known but unset keys in $phpns to null so that we don't have to
  // keep checking whether they're set every time we try to access them
  $phpns = $phpns + array(
    'always_show_extended_article' => null,
    'comment_override' => null,
    'content' => null,
    'def_comlimit' => null,
    'disable_extended_article' => null,
    'id' => null,
    'items' => null,
    'script_link' => null,
    'sef' => null,
  ...
  $phpns['do'] = isset($_POST['do']) ? htmlentities($_POST['do']) : null;
	if (!$phpns['do'] && isset($_GET['a']) && !strstr($_GET['a'], 'page:')) { $phpns['id'] = htmlentities($_GET['a']); }
	$phpns['mode'] = isset($phpns['mode']) ? htmlentities($phpns['mode']) : null;
	$phpns['offset'] = isset($phpns['offset']) ? htmlentities($phpns['offset']) : null;
	...
	$phpns['https_s'] = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on' ? 's' : '';
	...
	//define show_news functions, and check if functions are already defined.
		...
		*/
		...
		if (((!$phpns['do'] || $phpns['do'] == 'rss') && (!$phpns['id'] || $phpns['id'] == 'do=rss')) || $phpns['static'] == TRUE) { //if no defined action, show news as it is meant to be displayed.
			...
			if (isset($phpns['category'])) {
        $phpns['category'] = 'WHERE article_cat IN ('.$phpns['category'].',\'all\') &&';
      } else {
        $phpns['category'] = "WHERE"; 
      }

			if (!isset($phpns['offset'])) {
        $phpns['offset'] = db_fetch("SELECT * FROM ".$databaseinfo['prefix']."gconfig WHERE name='def_offset'",1);
        $phpns['offset'] = $phpns['offset']['v1'];
      }
      $phpns['original_offset'] = $phpns['offset']; //to be used later...

			if (!isset($phpns['limit'])) {
        $phpns['limit'] = db_fetch("SELECT * FROM ".$databaseinfo['prefix']."gconfig WHERE name='def_limit'",1);
        $phpns['limit'] = $phpns['limit']['v1'];
      }

			if (!isset($phpns['order'])) {
        $phpns['order'] = db_fetch("SELECT * FROM ".$databaseinfo['prefix']."gconfig WHERE name='def_order'",1);
        $phpns['order'] = $phpns['order']['v1'];
      }
  
			if (!isset($phpns['items_per_page'])) {
        $phpns['items_per_page'] = db_fetch("SELECT * FROM ".$databaseinfo['prefix']."gconfig WHERE name='def_items_per_page'",1);
        $phpns['items_per_page'] = $phpns['items_per_page']['v1'];
      }
				...
				if (isset($_GET['a']) && strstr($_GET['a'], "page:") && $phpns['static'] != TRUE) {
					//get the current page from the URI.
					$phpns['current_page'] = str_replace('page:','', $_GET['a']);
				}
			
					// if the page is not a number or nonpositive, we assume page 1.
					if (!isset($phpns['current_page']) || !is_numeric($phpns['current_page']) || $phpns['current_page'] < 1) {
